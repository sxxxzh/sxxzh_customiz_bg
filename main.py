...
class SystemTrayIcon:
    ...
    def on_edit_config(self, icon, item):
        ...
        try:
            ...
            if os.path.exists(config_file):
                ...
                # 显示提示信息
                log("配置文件已打开，修改后可能不能即时重载成功,请保存并重启程序生效")
    ...
    def on_clear_cache(self, icon, item):
        """清理缓存菜单项回调"""
        try:
            log("收到清理缓存指令，开始清理...")
            ok = False
            if hasattr(self.system, 'clear_cache_files'):
                ok = self.system.clear_cache_files()
            else:
                ok = False
            if ok:
                log("缓存清理完成")
    ...
    def on_rerender(self, icon, item):
        """重新渲染菜单项回调：强制重新扫描并重新挂载背景"""
        try:
            log("收到重新渲染指令，立即执行扫描与刷新...")
            if self.system and hasattr(self.system, 'window_detector') and self.system.window_detector:
                result = False
                if hasattr(self.system.window_detector, 'rerender_now'):
                    result = self.system.window_detector.rerender_now(full_reset=True)
                else:
                    # 兜底方案：尝试停止所有背景并触发一次扫描
                    try:
                        pm = getattr(self.system.window_detector, 'process_manager', None)
                        if pm and hasattr(pm, 'stop_all'):
                            pm.stop_all()
                    except Exception as e:
                        log(f"兜底停止背景进程失败: {e}")
                    try:
                        self.system.window_detector.scan_windows()
                        result = True
                    except Exception as e:
                        log(f"兜底扫描失败: {e}")
                        result = False
    ...
    def setup_menu(self):
        ...
        menu_items = [
            ...
            pystray.MenuItem("清理缓存", self.on_clear_cache),
            pystray.MenuItem("重新渲染", self.on_rerender),
...
class BackgroundSystem:
    ...
    def clear_cache_files(self):
        """清理所有缓存文件（尺寸缓存、渲染缓存、临时配置）"""
        try:
            log("开始清理所有缓存文件...")
            # 清理窗口尺寸缓存
            if self.window_detector and hasattr(self.window_detector, 'size_cache'):
                try:
                    self.window_detector.size_cache.clear_all(remove_file=True)
                except Exception as e:
                    log(f"清理窗口尺寸缓存失败: {e}")
            # 清理渲染缓存目录
            try:
                base_dir = os.path.dirname(sys.executable) if getattr(sys, 'frozen', False) else os.path.dirname(__file__)
                render_dir = os.path.join(base_dir, 'render_cache')
                if os.path.exists(render_dir) and os.path.isdir(render_dir):
                    for fn in os.listdir(render_dir):
                        ...
                        try:
                            ...
                            log(f"删除渲染缓存文件失败 {fp}: {e}")
                    log(f"渲染缓存目录已清理: {render_dir}")
            except Exception as e:
                log(f"清理渲染缓存目录失败: {e}")
            # 清理临时配置文件（第三层进程用）
            try:
                if self.window_detector and hasattr(self.window_detector, 'process_manager'):
                    self.window_detector.process_manager._cleanup_temp_files()
            except Exception as e:
                log(f"清理临时配置文件失败: {e}")
            log("所有缓存文件清理完成")
            return True
