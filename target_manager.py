...
def log(msg):
    """日志输出"""
    _log(msg, module="target-manager")
...
class TargetManager:
    ...
    def _validate_config(self, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # 检查 ignore_size 字段（窗口面积最小阈值）
            if 'ignore_size' not in config:
                config['ignore_size'] = 10000
            else:
                try:
                    config['ignore_size'] = int(config['ignore_size'])
                    if config['ignore_size'] < 0:
                        config['ignore_size'] = 10000
                except:
                    config['ignore_size'] = 10000
    ...
    def _validate_target_config(self, target: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # 可选：主窗口最小面积阈值
            if 'main_min_area' in target:
                try:
                    target['main_min_area'] = max(0, int(target['main_min_area']))
                except:
                    target['main_min_area'] = 0
